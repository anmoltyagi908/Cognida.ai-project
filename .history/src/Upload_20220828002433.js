...
export default function Upload() {
  ...
  const uploadFile = () => {
    ...
    const metadata = { creataDate: date };
  ...
  return (
    ...
    <input type="file" className="inputBox" placeholder="hello" onChange={(event) => {
      // event.preventDefault();
      setImageUpload(event.target.files[0]);
    }} />
    <button onClick={uploadFile} className="submitbutton">Upload</button>
